...
#cmc使用范例
data = c.cmc("300059.SZ", "OPEN,CLOSE,HIGH", (datetime.today() + timedelta(-6)).strftime("%Y-%m-%d"), datetime.today().strftime("%Y-%m-%d"),"Ispandas=0")
print u"cmc输出结果======分隔线======"
...
#实时行情订阅使用范例
data = c.csq("300059.SZ,002716.SZ,600834.SH,600616.SH", "TIME,OPEN,HIGH,LOW,NOW,AMOUNT", "Pushtype=1", csqCallback)
if data.ErrorCode != 0:
...
data = c.porder("quant001.PF", orderdict, "this is a test")
if(data.ErrorCode != 0):
...
#组合报表查询
data = c.preport("quant001.PF", "record", "startdate=2017/07/12,enddate=2018/01/15")
if(data.ErrorCode != 0):
