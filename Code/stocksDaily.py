...
def morningfall_select():
    ...
    title = ["股票代码", "股票名称", "当前涨跌幅", "开盘涨跌幅", "柱线幅度", "下影线幅度", "上影线幅度"]
    resultdata_list = []
    ...
    for item in noondata_list:
        ...
        if(cylinder_range<-3):
            resultdata_list.append([item[0], item[1], item[2], round(open_range,2), round(cylinder_range,2), round(downshadow_range,2), round(upshadow_range,2)])
