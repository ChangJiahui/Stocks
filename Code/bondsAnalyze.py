...
# slope + wave 模型
    ...
    title = ["可转债名称", "当日涨跌幅", "10日涨跌幅", "最近10日最大下跌幅度", "最近10日最大上涨幅度", "最近下跌幅度", "最近下跌天数", "最近上涨幅度", "最近上涨天数", "上一下跌幅度", "上一下跌天数", "上一上涨幅度", "上一上涨天数", "上二下跌幅度", "上二下跌天数", "上二上涨幅度", "上二上涨天数"]
    resultdata_list = []
    for filename in os.listdir(bonddata_path):
...
def slope_Model_Select_par():
    ...
    title = ["可转债名称", "当日涨跌幅", "10日涨跌幅", "最近10日最大下跌幅度", "最近10日最大上涨幅度", "最近下跌幅度", "最近下跌天数", "最近上涨幅度", "最近上涨天数", "上一下跌幅度", "上一下跌天数", "上一上涨幅度", "上一上涨天数", "上二下跌幅度", "上二下跌天数", "上二上涨幅度", "上二上涨天数"]
    pool = multiprocessing.Pool(4)
    resultdata_list = pool.map(slope_Model_Select_pipeline, os.listdir(bonddata_path))
    pool.close()
    pool.join()
    write_csvfile(resultfile_path, title, resultdata_list)
def slope_Model_Select_pipeline(filename):
    ...
    if((len(minprice_list)>3) and (len(maxprice_list)>3)):
        ...
        if((reboundrange<20) and (reboundcounter>3) and (sumrange<-10)):
            return [bondinfo, bonddata_list[0][8], slope_list[0], slope_list[minoffset_list[0]], slope_list[maxoffset_list[0]], reboundrange, reboundcounter, failrange, failcounter, lastreboundrange, lastreboundcounter, lastfailrange, lastfailcounter, penultreboundrange, penultreboundcounter, penultfailrange, penultfailcounter]
    return []
