...
resultfile_path = os.path.join(root_path, "Result", "vshape_Model_Select_Result.csv")
resultdata_path = os.path.join(root_path, "Result", "vshape_model_data")
...
def vshape_Model_Select():
    ...
    for filename in filenames:
        ...
        with open(filepath, "r") as fp:
            ...
            try:
                if((float(stock_data_list[1][9])<0) or (float(stock_data_list[1][10])<2*float(stock_data_list[2][10]))):
                    continue
            except:
                continue
            dropcounter = 0
